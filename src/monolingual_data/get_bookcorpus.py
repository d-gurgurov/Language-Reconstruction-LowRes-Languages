...
def load_and_prepare_data():
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Loading BookCorpus dataset...")
...
def save_detailed_statistics(df, output_dir):
    ...
    all_stats.to_csv(f'{output_dir}/bookcorpus_detailed_statistics.csv')
...
def create_enhanced_plots(df, output_dir):
    ...
    plt.savefig(f'{output_dir}/bookcorpus_distribution.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def calculate_ttr(df, output_dir):
    ...
    plt.savefig(f'{output_dir}/bookcorpus_ttr.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    ttr_results.to_csv(f'{output_dir}/bookcorpus_ttr_analysis.csv', index=False)

def main():
    start_time = time.time()
    
    # Create output directory
    output_dir = f"bookcorpus_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    os.makedirs(output_dir, exist_ok=True)
